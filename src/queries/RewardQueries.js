const keepRewardToHistory =
  "INSERT INTO Redeem_Histories (fk_customer_id, fk_reward_id, quantity, points_used) VALUES (?, ?, ?, ?)";
const getRedeemRewardTimestamp =
  "SELECT redeem_timestamp FROM Redeem_Histories WHERE fk_customer_id = ? ORDER BY redeem_history_id DESC LIMIT 1";
...
const addNewReward =
  "INSERT INTO Rewards (name, require_point, quantity, status, event_start_date, event_end_date, description,reward_type ,reward_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
const addCustomerGroupToReward =
  "INSERT INTO Reward_Customer_Groups (reward_id, group_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE reward_id = VALUES(reward_id)";
const decreaseReward =
  "UPDATE Rewards SET quantity = quantity - ? WHERE reward_id = ?";
const updateRewardDetails =
  "UPDATE Rewards SET name = ?, require_point = ?, quantity = ?, status = ?, event_start_date = ?, event_end_date = ?, description = ?,reward_type= ? WHERE reward_id = ?";
const updateRewardDetailsAndImage =
  "UPDATE Rewards SET name = ?, require_point = ?, quantity = ?, status = ?, event_start_date = ?, event_end_date = ?, description = ?,reward_type= ?, reward_image = ? WHERE reward_id = ?";
const getCustomerGroupOfReward =
  "SELECT * FROM Reward_Customer_Groups WHERE reward_id = ?";
const removeCustomerGroupFromReward =
  "DELETE FROM Reward_Customer_Groups WHERE reward_id = ? AND group_id = ?";
...
const getRewardsByEventTimeAndCustomerGroupView =
  "SELECT * FROM Rewards_event_Time_length_BY_Customer_Group_View WHERE group_id = ? AND status = 'Active'";
...
const getCouponCode =
  "SELECT coupon_code FROM Coupons WHERE fk_reward_id = ? AND is_activate = 0  LIMIT 1;";
const updateCouponStatus =
  "UPDATE Coupons SET is_activate = ? WHERE coupon_code = ?;";
const getLastedRedeemHistoryId =
  "SELECT redeem_history_id FROM Redeem_Histories WHERE fk_customer_id = ? ORDER BY redeem_timestamp DESC LIMIT 1;";
